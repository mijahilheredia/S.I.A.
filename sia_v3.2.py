...
class piso:
    ...
    def crear_apartamentos(self,nro):
        self.copy = 0
        while self.copy < nro:
            self.apar.append(apartamento("",0,0,False,self.copy))
            self.copy = self.copy+1
        self.__nro_apa = nro
...
class sia:
    ...
    def reconocer(self):
        ...
        with sr.Microphone() as source:
                print("te escucho")
                try:
                    self.__audio = self.__r.listen(source,5) #te escuchara por durante 5 segundos
...
#SIA ejecute alguna funcion
    ...
    def configuracion_inicial(self):
        ...
        self.setMensaje("¡¡¡BIENVENIDO A LA CONFIGURACION MANUAL ORIENTADA!!!")
        time.sleep(2)
        self.setMensaje("El sistema se ha iniciado por primera vez asi que comenzaremos ingresando el nombre del propietario\ny su respectiva contraseña \nIngrese el nombre completo del propietario")        
        self.__dato = input()
...
#define la configuracion  inicial de la casa
    def __configuracion_casa(self):
        ...
        pygame.mixer.music.play(1)
        print("Acabemos añadiendo algunos datos personales","Ingrese su fecha de nacimiento",sep='\n')
        self.__fn = input()
...
"""def window():
    ...
    vent = ventana.MyApp() #la interfaz grafica se encuentra en esta variable
    vent.enviarMensaje(s.getMensaje())
    vent.show()
    sys.exit(app.exec_())
...
"""if __name__ == "__main__":
    ...
    t1 = threading.Thread(name = 'hilo1',target=window)
    t2 = threading.Thread(name = 'hilo2',target=escucha)
...
s.crear_pisos(1)
s.piso[0].crear_apartamentos(1)
s.piso[0].apar[0].crear_sala(1)
s.piso[0].apar[0].sala[0].ejecutarOrden("Enciende la luz de la sala")
s.piso[0].apar[0].sala[0].establecer_seguridad("activar seguridad")
s.piso[0].apar[0].sala[0].reporte_puerta()
#s.escucharOrden()"""
